...
if sys.version_info >= (3, 0):
	xrange = range	# Python3's range() is python2's xrange()
...
class NeuronLayer():
	def __init__(self, neurons=0, inputs=0, predefined_weight=array(0)):
		if predefined_weight.all() == 0: 
			...
			# Load from a trained weights (load_weights is much preferred. Use this if you want to load weights for one layer only)
			# The value of 'neurons' and 'inputs' variables are ignored if you load weights this way
			self.synaptic_weight = predefined_weight
...
class NeuralNetwork():
	...
	def train(self, training_set_input, training_set_output, number_of_training_iterations, tolerance=0, learning_rate=1, verbose=0, steps=1):
		steps = xrange(0,number_of_training_iterations,steps)
		if len(training_set_input) == len(training_set_output):
			for iteration in xrange(number_of_training_iterations):
				...
				if sum_error < tolerance:
					# Breaks the loop if sum_error is less than tolerance
					break
				if verbose:
	...
	def load_weights(self,file_name):
		...
		if sys.version_info >= (3, 0):
			...
			# Load the archive
			archive = load(file_name)
		c = []
		for x in reversed(archive.files):
			# Load the weights from the files in the archive
			c.append(archive[x])
	...
	def save_weights(self, file_name):
		# Save the weights to an archive
		savez(file_name,*self.get_weights())
	...
	def from_file(file_name):
		...
		for x in reversed(archive.files):
			# Load the weights from the files in the archive
			layers.append(NeuronLayer(archive[x]))
